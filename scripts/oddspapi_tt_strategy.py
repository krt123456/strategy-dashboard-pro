    ...
    *,
    ...
    for _, row in df.iterrows():
        ...
        if prob_margin < min_margin:
            qualifies = False
        if odds is None or float(odds) > max_odds:
            qualifies = False
        if float(row.get("overround", 0.0) or 0.0) > max_overround:
            qualifies = False
...
def main() -> int:
    ...
    ap.add_argument("--out-picks", default="reports/oddspapi_tabletennis_picks.csv")
    ap.add_argument("--out-summary", default="reports/oddspapi_tabletennis_summary.csv")
