#!/usr/bin/env python3
"""List upcoming fixtures (with odds) for BetExplorer league list."""
...
def _extract_odds(block: str) -> tuple[str, str, str]:
...
def parse_fixtures_page(html: str) -> Iterable[Dict[str, str]]:
    ...
    for row in rows:
        ...
        odd1, odd2, odd3 = _extract_odds(row)
        yield {"Date": date_iso, "Home": teams[0], "Away": teams[1], "Odd1": odd1, "Odd2": odd2, "Odd3": odd3}
    ...
    for idx in range(len(starts) - 1):
        ...
        odd1, odd2, odd3 = _extract_odds(block)
        yield {"Date": date_iso, "Home": teams[0], "Away": teams[1], "Odd1": odd1, "Odd2": odd2, "Odd3": odd3}
...
def main() -> int:
    ...
    out_csv = Path(args.out_csv) if args.out_csv else Path(f"reports/upcoming_fixtures_{list_key}_odds_{start_date}_{end_date}.csv")
    out_md = Path(args.out_md) if args.out_md else Path(f"reports/upcoming_fixtures_{list_key}_odds_{start_date}_{end_date}.md")
    out_csv.parent.mkdir(parents=True, exist_ok=True)

    with out_csv.open("w", encoding="utf-8", newline="") as f:
        writer = csv.DictWriter(
            f, fieldnames=["League", "Date", "Home", "Away", "Odd1", "Odd2", "Odd3", "Source"]
        )
        writer.writeheader()
        writer.writerows(rows)

    lines = [
        "# Upcoming fixtures (BetExplorer + odds)",
        f"- List: {list_key}",
        f"- Range: {start_date.isoformat()} to {end_date.isoformat()}",
        f"- Total fixtures: {len(rows)}",
        "",
        "| League | Date | Home | Away | Odd1 | Odd2 | Odd3 | Source |",
        "| --- | --- | --- | --- | --- | --- | --- | --- |",
    ]
    for row in rows:
        lines.append(
            f"| {row['League']} | {row['Date']} | {row['Home']} | {row['Away']} | {row['Odd1']} | {row['Odd2']} | {row['Odd3']} | {row['Source']} |"
        )
