#!/usr/bin/env python3
"""Utilities for extracting results + odds from betexplorer.com (table tennis)."""
...
def parse_country_slugs(html: str) -> List[str]:
    slugs = set()
    for slug in re.findall(r'href="/table-tennis/([^/]+)/"', html):
        if slug in {"results", "fixtures", "odds"}:
            continue
        slugs.add(slug)
    return sorted(slugs)
...
def parse_country_leagues(html: str, country_slug: str) -> Dict[str, str]:
    pattern = rf'href="(/table-tennis/{re.escape(country_slug)}/[^\"]+/)"[^>]*>([^<]+)</a>'
...
def parse_season_links(html: str) -> List[Tuple[str, str]]:
    ...
    for rel_url, label in re.findall(r'<option value="([^"]+)">\s*([^<]+)</option>', html):
        if not rel_url.startswith("/table-tennis/"):
            continue
