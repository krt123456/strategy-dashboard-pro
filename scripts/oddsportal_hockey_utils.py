#!/usr/bin/env python3
"""Utilities for extracting hockey results + odds from OddsPortal."""
...
def _pick_odds(odds: Iterable[dict]) -> tuple[float | None, float | None, float | None]:
    by_type: Dict[str, Dict[int, float]] = {}
    ordered: Dict[str, List[float]] = {}
    for odd in odds or []:
        scope_id = odd.get("scopeId")
        if scope_id not in (None, 2):
            continue
        ...
        try:
            ...
            continue
        key = str(odd.get("bettingTypeId") or "0")
        ordered.setdefault(key, []).append(price_val)
        outcome = odd.get("outcomeResultId")
        if outcome in (0, 1, 2, 3):
            by_type.setdefault(key, {})[int(outcome)] = price_val

    best = None
    best_score = 0
    best_key = None
    for key, outcomes in by_type.items():
        if 1 in outcomes and 2 in outcomes:
            score = 2 + (1 if 0 in outcomes or 3 in outcomes else 0)
            if score > best_score:
                best_score = score
                best = outcomes
                best_key = key
    if best:
        odd_h = best.get(1)
        odd_a = best.get(2)
        odd_d = best.get(0, best.get(3))
        return odd_h, odd_d, odd_a

    # Fallback: use order of odds if outcome ids are missing or unusable.
    for key, seq in ordered.items():
        if len(seq) >= 2:
            odd_h = seq[0]
            odd_a = seq[-1]
            odd_d = seq[1] if len(seq) >= 3 else None
            return odd_h, odd_d, odd_a
...
def _parse_match_rows(rows: Iterable[dict], season_label: str) -> Iterable[MatchRow]:
    for row in rows:
        ...
        try:
            ...
            continue
        odd_h, odd_d, odd_a = _pick_odds(row.get("odds", []) or [])
...
def download_league_csv(
    ...
    *,
    ...
    with out_path.open("w", encoding="utf-8", newline="") as f:
        writer = csv.writer(f)
        writer.writerow(["Date", "HomeTeam", "AwayTeam", "FTHG", "FTAG", "AvgH", "AvgD", "AvgA", "Season"])
        for row in rows:
            writer.writerow(
                [
                    row.date_iso,
                    ...
                    row.ftag,
                    row.odd_h,
                    row.odd_d if row.odd_d is not None else "",
                    row.odd_a,
                    row.season,
                ]
            )
