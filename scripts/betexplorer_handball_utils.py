#!/usr/bin/env python3
"""Utilities for extracting results + 1X2 odds from betexplorer.com (handball)."""
...
def parse_country_leagues(html: str, country_slug: str) -> Dict[str, str]:
    pattern = rf'href="(/handball/{re.escape(country_slug)}/[^"]+/)"[^>]*>([^<]+)</a>'
...
def parse_season_links(html: str) -> List[Tuple[str, str]]:
    ...
    for rel_url, label in re.findall(r'<option value="([^"]+)">\s*([^<]+)</option>', html):
        if not rel_url.startswith("/handball/"):
            continue
